...
describes what users get. Pick a format with `-o`, or let acmq choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
acmq <command> -o table   # aligned columns for reading
acmq <command> -o jsonl   # one JSON object per line, for piping
acmq <command> -o json    # a single JSON array
acmq <command> -o csv     # spreadsheet friendly
acmq <command> -o tsv     # tab-separated
acmq <command> -o url     # just the URL column
acmq <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
acmq <command> --fields id,title,url
```
...
```bash
acmq <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
acmq <command>            # a table, because this is a terminal
acmq <command> | wc -l    # JSONL, because this is a pipe
```
