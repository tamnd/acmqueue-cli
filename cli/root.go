// Package cli builds the acmq command tree on top of the acmqueue library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: acmqueue.DefaultConfig()}

	root := &cobra.Command{
		Use:   "acmq",
		Short: "Read ACM Queue practitioner technology articles.",
		Long: `acmq reads ACM Queue through its public RSS feed.

The feed is open and requires no API key. It returns the 20 most recent
articles with title, date, and URL. Records are printed as table, JSON, JSONL,
CSV, TSV, or URLs.

acmq is an independent tool and is not affiliated with ACM or ACM Queue.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = acmqueue.NewClient(a.cfg)
	return nil
}
